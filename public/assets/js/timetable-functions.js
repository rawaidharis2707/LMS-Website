...
if (typeof window.TIMETABLE_STORAGE_KEY === 'undefined') {
    window.TIMETABLE_STORAGE_KEY = 'lms_timetables';
}
if (typeof window.TIMETABLE_SETTINGS_KEY === 'undefined') {
    window.TIMETABLE_SETTINGS_KEY = 'lms_timetable_settings';
}
...
window.initTimetableStorage = function() {
...
// Get all entries
window.getAllTimetableEntries = function() {
...
window.getTimetableForClass = function(className, day) {
    const entries = window.getAllTimetableEntries();
    return entries
        ...
        .map(e => ({
            ...e,
            time: `${window.formatTime(e.startTime)} - ${window.formatTime(e.endTime)}` // Helper
        }));
}
