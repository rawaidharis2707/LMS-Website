...
<body class="dashboard-body">
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-menu">
            <ul class="list-unstyled">
                ...
                <li class="sidebar-menu-item"><a href="timetable.html" class="sidebar-link active"><i
    ...
    <main class="main-content">
        <nav class="top-navbar">
            <div class="d-flex align-items-center">
                ...
                <h1 class="page-title mb-0">Academic Schedule</h1>
            </div>
        ...
        <div class="dashboard-content px-4 py-4">
            <div class="card-modern animate-fade-in shadow-lg overflow-hidden border-0">
                <div
                    class="card-header bg-white border-0 py-4 px-4 border-bottom d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 fw-bold text-dark"><i class="fas fa-calendar-alt me-2 text-primary"></i> Faculty
                        Timetable</h5>
                    <div class="d-flex gap-2">
                        <button class="btn btn-primary btn-sm lift px-3" onclick="window.print()">
                        ...
                        <table class="table table-bordered mb-0 border-0">
                            <thead class="bg-light text-muted small text-uppercase">
                                <tr class="border-top-0">
                                    <th class="ps-4 border-0 text-center" style="width: 20%;">Monday</th>
                                    <th class="text-center border-0" style="width: 20%;">Tuesday</th>
                                    <th class="text-center border-0" style="width: 20%;">Wednesday</th>
                                    <th class="text-center border-0" style="width: 20%;">Thursday</th>
                                    <th class="text-center border-0" style="width: 20%;">Friday</th>
                                </tr>
                            </thead>
    ...
    <script>
        ...
        function loadTeacherTimetable() {
            ...
            const teacherName = session && session.userData ? session.userData.name : '';

            const allEntries = getAllTimetableEntries();
            const teacherEntries = allEntries.filter(e => e.teacher === teacherName);

            teacherEntries.sort((a, b) => (a.startTime > b.startTime) ? 1 : -1);
            ...
            ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].forEach(day => {
                ...
                html += '<td class="p-3 align-top bg-light-subtle">';

                if (dayEntries.length > 0) {
                    dayEntries.forEach(e => {
                        const timeStr = e.time || `${formatTime(e.startTime)} - ${formatTime(e.endTime)}`;
                        const color = subjectColors[e.subject] || '#4f46e5';

                        html += `
                            <div class="mb-3 p-3 rounded-4 shadow-sm bg-white animate-fade-in hover-lift border-start border-4" style="border-color: ${color} !important;">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                     <div class="badge bg-light text-dark border font-xs fw-bold">${timeStr}</div>
                                     <div class="small text-muted fw-bold">${e.room}</div>
                                </div>
                                <div class="fw-bold text-dark mb-1 h6">${e.subject}</div>
                                <div class="d-flex align-items-center text-muted small">
                                    <i class="fas fa-users me-2"></i> ${e.className}
                                </div>
                            </div>
                         `;
                    });
                    ...
                    html += `
                        <div class="text-center py-5 opacity-25">
                            ...
                            <div class="small fw-bold text-uppercase">Free</div>
                        </div>
                     `;
                }
