    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-menu">
            <ul class="list-unstyled">
                ...
                <li class="sidebar-menu-item"><a href="{{ url('admin/class-management.html') }}" class="sidebar-link active"><i
                        ...
                        <div class="card-modern animate-fade-in shadow-lg border-0 h-100">
                            ...
                            <div class="card-body p-4">
                                <form id="classForm">
                                    ...
                                    <button type="submit" class="btn btn-primary w-100 py-3 lift fw-bold shadow-sm">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('classForm').addEventListener('submit', async function (e) {
                ...
                const batchName = document.getElementById('classBatchInput').value.trim();
                const level = document.getElementById('educationLevel').value;
                const group = document.getElementById('classGroup').value;

                const result = await addClass(name, section, capacity, batchName, level, group);
                ...
                classes.forEach(cls => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td>
                            ...
                            <small class="text-muted d-block mt-1">${cls.capacity || 0} Seats</small>
                        </td>
        ...
        async function updateClassStats() {
            try {
                ...
                document.getElementById('totalClasses').innerText = classes.length;
                document.getElementById('totalEnrolled').innerText = studentsRes.length;

                if (classes.length > 0) {
                    const totalCap = classes.reduce((acc, curr) => acc + (parseInt(curr.capacity) || 0), 0);
                    document.getElementById('meanCapacity').innerText = Math.round(totalCap / classes.length);
                } else {
