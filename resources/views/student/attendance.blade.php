...
<body class="dashboard-body">
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-menu">
            <ul class="list-unstyled">
                ...
                <li class="sidebar-menu-item"><a href="{{ url('student/attendance') }}" class="sidebar-link active"><i
        ...
        <div class="dashboard-content">
                ...
                <div class="col-md-12">
                    <div class="card-modern animate-fade-in">
                        <div class="card-body text-center p-5">
                            ...
                            <h2 class="fw-bold mb-2">Overall Attendance</h2>
                            <h1 class="display-3 fw-bold text-success mb-3">92.5%</h1>
                            <div class="progress mb-3" style="height: 30px;">
                                <div class="progress-bar bg-success" style="width: 92.5%;">92.5%</div>
                            </div>
                            <p class="text-muted" id="attendanceMessage">You're doing great! Keep it up!</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-modern animate-fade-in mb-4">
                <div class="card-header bg-white border-0 py-3">
                    <h5 class="mb-0 fw-bold"><i class="fas fa-chart-pie text-primary me-2"></i> Subject-Wise Attendance
                    </h5>
                </div>
                <div class="card-body">
            ...
            <div class="row g-4 mb-4">
                <div class="col-lg-8">
                    <div class="card-modern animate-fade-in h-100">
                        <div class="card-header bg-white border-0 py-3">
                            <h5 class="mb-0 fw-bold"><i class="fas fa-calendar-check text-info me-2"></i> Monthly Trend
                            </h5>
                        </div>
                        <div class="card-body">
                ...
                <div class="col-lg-4">
                    <div class="card-modern animate-fade-in h-100">
                        <div class="card-header bg-white border-0 py-3">
                            <h5 class="mb-0 fw-bold"><i class="fas fa-chart-pie text-primary me-2"></i> Subject Analysis
                            </h5>
                        </div>
                        <div class="card-body" style="height: 300px;">
                            <canvas id="subjectAttendanceRadar"></canvas>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            initDashboardUser();
            initAttendanceStorage();
            calculateAndLoadAttendance();
        });

        function calculateAndLoadAttendance() {
            const currentUser = getCurrentUser();
            const studentId = currentUser && currentUser.id ? currentUser.id : '2024-10A-015';
            ...
            if (total === 0) {
                const overallEl = document.querySelector('.display-3');
                const progressBar = document.querySelector('.progress-bar');
                const messageEl = document.getElementById('attendanceMessage');
                const iconEl = document.querySelector('.card-body .fas.fa-user-check');

                if (overallEl) overallEl.innerText = "100%";
                if (progressBar) {
                ...
                if (iconEl) {
                    iconEl.className = "fas fa-user-graduate fa-5x text-success animate-bounce";
                }
                if (messageEl) {
                    messageEl.innerText = "Welcome! Your attendance journey begins now.";
                    messageEl.className = "text-success fw-bold";
                }
            } else {
                let overallPct = (present / total) * 100;
                overallPct = Math.round(overallPct * 10) / 10;

                const overallEl = document.querySelector('.display-3');
                if (overallEl) overallEl.innerText = `${overallPct}%`;
                ...
                if (progressBar) {
                    ...
                    progressBar.innerText = `${overallPct}%`;

                    progressBar.classList.remove('bg-success', 'bg-warning', 'bg-danger', 'progress-bar-striped', 'progress-bar-animated');
                    if (iconEl) iconEl.classList.remove('text-success', 'text-warning', 'text-danger', 'fa-user-graduate', 'animate-bounce');
                    if (iconEl && !iconEl.classList.contains('fa-user-check')) iconEl.className = "fas fa-user-check fa-5x";
                    ...
                    if (overallPct >= 90) {
                        ...
                        message = "You're doing okay, but try not to miss more classes.";
                    } else {
                        colorClass = 'bg-danger';
                        if (iconEl) iconEl.classList.add('text-danger');
                        message = "Warning: Your attendance is low. Please improve immediately.";
                    }

                    progressBar.classList.add(colorClass);
                    if (messageEl) {
                        messageEl.innerText = message;
                        messageEl.className = overallPct >= 75 ? 'text-muted' : 'text-danger fw-bold';
                    }
                }
            }
            ...
            const tbody = document.getElementById('subjectAttendanceTable');
            let html = '';

            if (Object.keys(subjects).length === 0) {
                html = '<tr><td colspan="5" class="text-center text-muted">No attendance records found.</td></tr>';
            } else {
                for (const [subName, data] of Object.entries(subjects)) {
                    const subPct = (data.present / data.total) * 100;
                    const statusBadge = subPct >= 90 ? '<span class="badge bg-success">Excellent</span>' :
                        subPct >= 75 ? '<span class="badge bg-warning">Good</span>' :
                            '<span class="badge bg-danger">Low</span>';
                    const colorClass = subPct >= 90 ? 'bg-success' : subPct >= 75 ? 'bg-warning' : 'bg-danger';

                    html += `
                        <tr>
                            <td class="fw-bold">${subName}</td>
                            <td>${data.present}</td>
                            <td>${data.total}</td>
                            <td>
                                <div class="progress" style="height: 20px;">
                                    <div class="progress-bar ${colorClass}" style="width: ${subPct}%;">${subPct.toFixed(1)}%</div>
                                </div>
                            </td>
                            <td>${statusBadge}</td>
                        </tr>
            ...
            loadMonthlyTrend(records);
            renderSubjectRadar(subjects);
        }

        function renderSubjectRadar(subjectsData) {
            const ctx = document.getElementById('subjectAttendanceRadar');
            if (!ctx) return;

            const existingChart = Chart.getChart(ctx);
            if (existingChart) existingChart.destroy();

            let labels = Object.keys(subjectsData);
            let dataPoints = labels.map(sub => {
                const d = subjectsData[sub];
                return Math.round((d.present / d.total) * 100);
            });

            if (labels.length === 0) {
                labels = ['Math', 'Physics', 'Chemistry', 'Biology', 'English', 'Computer'];
                dataPoints = [85, 90, 75, 88, 92, 95];

                const titleEl = document.querySelector('#subjectAttendanceRadar').closest('.card-body').previousElementSibling.querySelector('h5');
                if (titleEl) titleEl.innerHTML = '<i class="fas fa-chart-pie text-primary me-2"></i> Subject Analysis <span class="badge bg-soft-info text-info small ms-2">Demo View</span>';
            } else {
                const titleEl = document.querySelector('#subjectAttendanceRadar').closest('.card-body').previousElementSibling.querySelector('h5');
                if (titleEl) titleEl.innerHTML = '<i class="fas fa-chart-pie text-primary me-2"></i> Subject Analysis';
            }
        ...
        function loadMonthlyTrend(records) {
            ...
            records.forEach(r => {
                ...
                if (date.getFullYear() === currentYear) {
                    ...
                    if (!monthStats[mIndex]) monthStats[mIndex] = { present: 0, total: 0 };
                    monthStats[mIndex].total++;
                    if (r.status === 'Present') monthStats[mIndex].present++;
                }
            });
            ...
            const activeMonths = Object.keys(monthStats).sort((a, b) => a - b);

            if (activeMonths.length === 0) {
                container.innerHTML = '<p class="text-center text-muted">No data available for trend analysis.</p>';
                return;
            }

            activeMonths.forEach(mIdx => {
                const stat = monthStats[mIdx];
                const pct = Math.round((stat.present / stat.total) * 100);

                html += `
                    <div class="col-md-2 col-4">
                        <div class="text-center">
                            <div class="mb-2 fw-bold small">${months[mIdx]}</div>
                            <div class="progress" style="height: 100px; width: 20px; margin: 0 auto; background-color: #e9ecef;">
                                <div class="progress-bar bg-primary" style="width: 100%; height: ${pct}%;" 
                                     data-bs-toggle="tooltip" title="${pct}%"></div>
                            </div>
                            <div class="mt-2 small fw-bold">${pct}%</div>
                        </div>
                    </div>
                `;
            });
