...
<head>
    ...
    <title>Time Table - Student Portal</title>
...
<body class="dashboard-body">
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-menu">
            <ul class="list-unstyled">
                ...
                <li class="sidebar-menu-item"><a href="{{ url('student/timetable') }}" class="sidebar-link active"><i
    ...
    <main class="main-content">
        <nav class="top-navbar">
            <div class="d-flex align-items-center">
                ...
                <h1 class="page-title mb-0">Time Table</h1>
            </div>
        ...
        <div class="dashboard-content">
            <div class="card-modern animate-fade-in mb-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h5 class="mb-1 fw-bold">Weekly Schedule</h5>
                            <p class="text-muted mb-0">Class 10-A | Academic Year 2024</p>
                        </div>
                        <div>
                            <button class="btn btn-outline-primary" onclick="printElement('timetableContent')">
            ...
            <div class="card-modern animate-fade-in" id="timetableContent">
            ...
            <div class="card-modern animate-fade-in mt-4">
                <div class="card-header bg-white">
                    <h6 class="mb-0 fw-bold">Subject Color Legend</h6>
                </div>
                <div class="card-body">
                    <div class="row" id="colorLegend">
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function loadTimetable() {
            ...
            const userClassId = user.school_class_id || 1; // Default to 1 if not set
            ...
            for (const day of days) {
                const dayEntries = await getTimetableForClass(userClassId, day);

                html += '<td class="p-3 align-top bg-light-subtle" style="width: 20%;">';

                if (dayEntries.length > 0) {
                    dayEntries.forEach(e => {
                        const timeStr = `${e.start_time.substring(0, 5)} - ${e.end_time.substring(0, 5)}`;
                        const subjectName = e.subject ? e.subject.name : 'Unknown';
                        const teacherName = e.teacher ? e.teacher.name : 'Unknown';
                        const color = subjectColors[subjectName] || '#adb5bd';
                        ...
                        html += `
                            <div class="mb-3 p-3 rounded-4 shadow-sm bg-white animate-fade-in hover-lift border-start border-4" style="border-color: ${color} !important;">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                     ...
                                     <div class="small text-muted fw-bold"><i class="fas fa-door-open me-1"></i>${e.room || 'N/A'}</div>
                                </div>
                                <div class="fw-bold text-dark mb-1 h6" style="color: ${color};">${subjectName}</div>
                                <div class="small text-muted"><i class="fas fa-user-tie me-1"></i> ${teacherName}</div>
                            </div>
                         `;
                    });
                } else {
